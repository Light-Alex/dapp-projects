...
# 产品需求
内部版本没有kyc，注册流程
NovaLaunch 是一个区块链上的一个去中心化发行平台，专注于启动和支持新项目。它提供了一个平台，允许新的和现有的项目通过代币销售为自己筹集资金，同时也为投资者提供了一个参与初期项目投资的机会。下面是NovaLaunch 产品流程的大致分析：
1. 项目申请和审核
- 申请：项目方需要在NovaLaunch 上提交自己项目的详细信息，包括项目介绍、团队背景、项目目标、路线图、以及如何使用筹集的资金等。
- 审核：NovaLaunch 团队会对提交的项目进行审核，评估项目的可行性、团队背景、项目的创新性、以及社区的兴趣等。这一过程可能还包括与项目方的面对面或虚拟会议。
2. 准备代币销售
- 设置条款：一旦项目被接受，NovaLaunch 和项目方将协商代币销售的具体条款，包括销售类型（如公开销售或种子轮）、价格、总供应量、销售时间等。
- 准备市场：同时，项目方需要准备营销活动来吸引潜在的投资者。NovaLaunch 也可能通过其平台和社区渠道为项目提供曝光。
3. KYC和白名单
...
# 项目说明
该代码仓库共分为两个项目，分别为Farm（质押挖矿,流动性挖矿）与 Sale（项目IDO)，流程如下。
...
合约FarmingNovaLaunch具体实现了上述过程，以下是该合约的一些关键点：
...
2. 部署NovaLaunch token
   `npx hardhat run scripts/deployment/deploy_NovaLaunch_token.js --network sepolia`

3. 部署airdrop合约
   `npx hardhat run scripts/deployment/deploy_airdrop_NovaLaunch.js --network sepolia`

4. 修改前端地址，运行前端测试airdrop功能

进入前端目录NovaLaunch-fe，安装依赖
`yarn`

修改token地址和airdrop合约地址为合约之前部署的两个地址，如下：

NovaLaunch-fe/src/config/index.js 中的
`AIRDROP_TOKEN_ADDRESS_MAP` 的 31337（本地链端口）地址修改为NovaLaunch-TOKEN的地址
`AIRDROP_CONTRACT` Airdrop-NovaLaunch的地址
...
链ID为13337
NovaLaunch-fe/src/util/chain_id.ts
NovaLaunch-fe/src/config/valid_chains.js
如有更换，在这两个文件中修改本地链ID和rpc地址

6. farm

修改NovaLaunch-contracts/scripts/deployment/deploy_farm.js
第7行startTS为3分钟之后（必须是当前时间之后，考虑上链网络延迟）

修改 NovaLaunch-fe/src/config/farms.js
...
## 具体操作
1. Farm 流程需要用到我们的Erc20测试代币NovaLaunch, 可以在首页领取NovaLaunch(一个账户只能领取一次),并且添加到我们metamask，添加之后我们可以在metamask 看到我们领取的NovaLaunch 代币

2. 在我们farm界面，我们可以质押fNovaLaunch 代币获取NovaLaunch, (方便大家操作，我们的测试网fNovaLaunch，NovaLaunch 是在上一步中领取的同一代币)，在这里我们有三个操作，stake:质押，unstake(withdraw):撤回质押， 以及 claim:领取奖励;

点击stake 或者claim 进入对应的弹窗，切换tab可以进行对应的操作；
3. Stake ，输入要质押的FNovaLaunch代币数量，点击stake 会唤起钱包，在钱包中confirm，然后等待交易完成；

我们新增质押了1FNovaLaunch,交易完成之后我们会看到，My staked 从0.1 变成1.1;
Total staked 的更新是一个定时任务，我们需要等待一小段时间之后才能看到更新

3. Claim 领取质押奖励的NovaLaunch,点击claim 并且在钱包确认

交易完成后我们会看到Available的FNovaLaunch数量增加了96，钱包里面NovaLaunch的代币数量同样增加了96

4. Unstake(withdraw),输入需要撤回的FNovaLaunch 数量(小于已经质押的Balance)，点击withdraw，并且在钱包确认交易

unstake 完成后我们可以看到my staked 的数量变为0

## 概述
FarmingNovaLaunch合约是一个基于以太坊的智能合约，主要用于管理和分发基于用户质押的流动性证明(LP)代币的ERC20奖励。该合约允许用户存入LP代币，并根据质押的数量和时间来计算和分发ERC20类型的奖励。
...
## NovaLaunch-contacts Makefile命令解释
