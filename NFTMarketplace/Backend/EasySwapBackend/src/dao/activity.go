...
// removeRepeatedElement 从字符串数组中移除重复的元素，并返回一个新的数组
// 参数:
//   arr []string: 包含重复元素的字符串数组
// 返回值:
//   newArr []string: 不包含重复元素的新数组
func removeRepeatedElement(arr []string) (newArr []string) {
	// 初始化一个新的空数组用于存储不重复的元素
	...
	for i := 0; i < len(arr); i++ {
		...
		for j := i + 1; j < len(arr); j++ {
			// 如果找到与当前元素相同的元素，则标记为重复，并跳出内层循环
			if arr[i] == arr[j] {
				repeat = true
				break
			}
		}
		// 如果当前元素没有重复且不为空，则将其添加到新数组中
		if !repeat && arr[i] != "" {
...
// removeRepeatedElementArr 函数用于从二维字符串数组中移除重复的键值对
//
// 参数：
//     arr [][]string: 二维字符串数组，每个子数组包含两个或三个字符串，表示键值对
//
// 返回值：
//     [][]string: 移除重复键值对后的二维字符串数组
