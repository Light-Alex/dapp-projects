...
const Uint128Max = ethers.parseUnits(
  "340282366920938463463.374607431768211455",
  18
);
const Uint256Max = ethers.parseUnits(
...
describe("EasySwap Test", function () {
  beforeEach(async function () {
    ...
    newESVault = await esVault.getAddress();
    ...
    testERC721.setApprovalForAll(await esVault.getAddress(), true);
    // testERC721.setApprovalForAll(esDex.address, true)

    await esVault.setOrderBook(await esDex.getAddress());
  });
  ...
  describe("should make order successfully", async () => {
    it("should make list/sell order successfully", async () => {
      ...
      const salt = 1;
      const nftAddress = await testERC721.getAddress();
      const tokenId = 0;
      const nftAmount = 1;
      const order = {
        side: Side.List,
        ...
        price: ethers.parseUnits("0.01", 18), //toBn("0.01"),
        expiry: now,
        salt: salt,
      };
      const orders = [order];

      orderKeys = await esDex.makeOrders.staticCall(orders);
      ...
      await expect(await testERC721.ownerOf(0)).to.equal(
    ...
    it("should make list/sell order and return orders successfully", async () => {
      ...
      orderKeys = await esDex.makeOrders(orders);
      expect(orderKeys[0]).to.not.equal(Byte32Zero);
    });
    ...
    it("should make bid/buy order successfully", async () => {
      ...
      orderKeys = await esDex.makeOrders.staticCall(orders, {
        value: ethers.parseUnits("0.02", 18),
      });
      expect(orderKeys[0]).to.not.equal(Byte32Zero);

      await expect(
        await esDex.makeOrders(orders, { value: ethers.parseUnits("0.02", 18) })
      ).to.changeEtherBalances(
        [owner, esVault],
        [ethers.parseUnits("-0.01", 18), ethers.parseUnits("0.01", 18)]
      );
    ...
    it("should make two side order successfully", async () => {
      ...
      expect(dbOrder.order.maker).to.equal(owner.address);
      expect(await testERC721.ownerOf(0)).to.equal(await esVault.getAddress());
  ...
  describe("should cancel order successfully", async () => {
    it("should cancel list order successfully", async () => {
      ...
      console.log("dbOrder: ", dbOrder);
      expect(dbOrder.order.maker).to.equal(owner.address);
      // 使用 callStatic 预先检查返回值
      console.log("esDex.staticCall: ", esDex.cancelOrders.staticCall);
      const successes = await esDex.cancelOrders.staticCall([orderHash]);
      ...
      // console.log("txRec: ", txRec.logs)
      // 执行实际的取消操作
      await expect(await esDex.cancelOrders([orderHash])).to.emit(
        esDex,
        "LogCancel"
      );
      // 验证订单已被取消
    ...
    it("should cancel bid order successfully", async () => {
      ...
      await expect(
        await esDex.makeOrders(orders, { value: ethers.parseUnits("0.07", 18) })
      ).to.changeEtherBalances(
        [owner, esVault],
        [ethers.parseUnits("-0.05", 18), ethers.parseUnits("0.05", 18)]
      );
      ...
      successes = await esDex.cancelOrders.staticCall([orderHash]);
      ...
      await expect(
        await esDex.cancelOrders([orderHash])
      ).to.changeEtherBalances(
        [owner, esVault],
        [ethers.parseUnits("0.05", 18), ethers.parseUnits("-0.05", 18)]
      );
    ...
    async function perparePartlyFilledOrder() {
      ...
      await expect(
        await esDex
          .connect(addr1)
          .makeOrders([buyOrder], { value: ethers.parseUnits("0.04", 18) })
      ).to.emit(esDex, "LogMake");
      ...
      salt = 2;
      nftAddress = await testERC721.getAddress();
      tokenId = 1;
      sellOrder = {
        side: Side.List,
        saleKind: SaleKind.FixedPriceForItem,
      ...
      await expect(
        await esDex.matchOrder(sellOrder, buyOrder)
      ).to.changeEtherBalances(
        [esDex, owner, esVault],
        [
          ethers.parseUnits("0.0002", 18),
          ethers.parseUnits("0.0098", 18),
    ...
    it("should cancel bid order partly filled successfully", async () => {
      ...
      await expect(
        await esDex.connect(addr1).cancelOrders([orderHash])
      ).to.emit(esDex, "LogCancel");
      ...
      expect(newETHBalance).to.equal(ethers.parseUnits("0", 18));
    });
  });
  ...
  describe("should edit orders successfully", async () => {
    it("should edit list orders successfully", async () => {
      ...
      order2 = {
        ...
        price: ethers.parseUnits("0.02", 18),
        expiry: now,
        salt: salt,
      };
      ...
      newOrder2 = {
        ...
        maker: owner.address,
        nft: [tokenId2, nftAddress, 1],
        price: ethers.parseUnits("0.04", 18),
        expiry: now,
        salt: 11,
      };
      ...
      newOrderKeys = await esDex.editOrders.staticCall(editDetails);
      ...
      newOrderKeys = await esDex.editOrders.staticCall(editDetailsSkip);
      expect(newOrderKeys[0]).to.not.equal(Byte32Zero);
    ...
    it("should edit bid order successfully, all new price > old price", async () => {
      ...
      await expect(
        await esDex.makeOrders(orders, { value: toBn("0.04") })
      ...
      await expect(
        await esDex.editOrders(editDetails, { value: toBn("0.1") })
      ...
      expect(newETHBalance).to.equal(ethers.parseUnits("0.04", 18));
      ...
      expect(newETHBalance2).to.equal(ethers.parseUnits("0.06"));
    ...
    it("should edit bid order successfully, all new price < old price", async () => {
      ...
      await expect(
        await esDex.makeOrders(orders, { value: ethers.parseUnits("0.04") })
      ).to.changeEtherBalances(
        [owner, esVault],
        [ethers.parseUnits("-0.02"), toBn("0.02")]
      );
      ...
      await expect(
        ...
        [owner, esVault],
        [ethers.parseUnits("-0.025"), ethers.parseUnits("0.025")]
      );
      ...
      newETHBalance = await esVault.ETHBalance(newOrderHash);
      expect(newETHBalance).to.equal(ethers.parseUnits("0.015"));
      oldETHBalance = await esVault.ETHBalance(orderHash);
      expect(oldETHBalance).to.equal(0);

      newETHBalance2 = await esVault.ETHBalance(newOrder2Hash);
      expect(newETHBalance2).to.equal(ethers.parseUnits("0.03"));
    ...
    it("should edit bid order successfully, order one: new price < old price, order two: new price > old price", async () => {
      ...
      await expect(
        ...
        [owner, esVault],
        [ethers.parseUnits("-0.02"), ethers.parseUnits("0.02")]
      );
      ...
      await expect(
        ...
        [owner, esVault],
        [ethers.parseUnits("-0.026"), ethers.parseUnits("0.026")]
      );
      ...
      newETHBalance = await esVault.ETHBalance(newOrderHash);
      expect(newETHBalance).to.equal(ethers.parseUnits("0.04"));
      oldETHBalance = await esVault.ETHBalance(orderHash);
      expect(oldETHBalance).to.equal(0);

      newETHBalance2 = await esVault.ETHBalance(newOrder2Hash);
      expect(newETHBalance2).to.equal(ethers.parseUnits("0.006"));
  ...
  describe("should match order successfully", async () => {
    describe("should check match available successfully", async () => {
      it("should match list order successfully", async () => {
        ...
        await expect(await esDex.makeOrders([order])).to.emit(esDex, "LogMake");
        ...
        await expect(
          await esDex
            .connect(addr1)
            .matchOrder(order, buyOrder, { value: ethers.parseUnits("0.03") })
        ).to.changeEtherBalances(
          [esDex, owner, addr1],
          [
            ethers.parseUnits("0.0002"),
            ethers.parseUnits("0.0098"),
            ethers.parseUnits("-0.01"),
          ]
        );
        expect(await testERC721.ownerOf(0)).to.equal(addr1.address);

        // tx = await esDex.connect(addr1).matchOrder(order, buyOrder, { value: ethers.parseUnits("0.01") })
        // txRec = await tx.wait()
        // console.log("txRec: ", txRec.logs)
      ...
      it("should match collection bid order successfully", async () => {
        ...
        await expect(
          await esDex
            .connect(addr1)
            .makeOrders([buyOrder], { value: ethers.parseUnits("0.04") })
        ).to.emit(esDex, "LogMake");
...

        {
          ...
          sellOrder = {
            ...
            maker: owner.address,
            nft: [tokenId, nftAddress, 1],
            price: ethers.parseUnits("0.01", 18), //ethers.parseUnits("0.01"),
            expiry: now,
            salt: salt,
          };
          ...
          await expect(
            ...
            [esDex, owner, esVault],
            [ethers.parseUnits("0.0002"), toBn("0.0098"), toBn("-0.01")]
          );
          expect(await testERC721.ownerOf(1)).to.equal(addr1.address);
...

        {
          ...
          await expect(
            esDex.matchOrder(sellOrder, buyOrder)
      ...
      it("should match item bid order successfully", async () => {
        ...
        await expect(
          await esDex
            .connect(addr1)
            .makeOrders([buyOrder], { value: toBn("0.01") })
        ).to.emit(esDex, "LogMake");
      ...
      it("should revert if order is the same", async () => {
        ...
        await expect(
          esDex
            .connect(addr1)
            .matchOrder(order, buyOrder, { value: toBn("0.01") })
      ...
      it("should revert if list order was filled", async () => {
        ...
        await expect(
          await esDex
            .connect(addr1)
            .matchOrder(order, buyOrder, { value: toBn("0.03") })
        ).to.changeEtherBalances(
          [esDex, owner, addr1],
          [toBn("0.0002"), toBn("0.0098"), toBn("-0.01")]
        );
      ...
      it("should revert if bid order was filled", async () => {
        ...
        await expect(
          await esDex
            .connect(addr1)
            .makeOrders([buyOrder], { value: toBn("0.02") })
        ).to.emit(esDex, "LogMake");
        ...
        await expect(
          await esDex.matchOrder(sellOrder2, buyOrder)
        ).to.changeEtherBalances(
          [esDex, owner, esVault],
          [toBn("0.0002"), toBn("0.0098"), toBn("-0.01")]
        );
        ...
        await expect(esDex.matchOrder(sellOrder2, buyOrder)).to.be.revertedWith(
          "HD: order closed"
        );
      });
    });
  ...
  describe("should check match successfully if msg.sender is sellOrder.maker", async () => {
    ...
    beforeEach(async function () {
      ...
      orders = [bidOrder];
      await expect(
        await esDex.connect(addr1).makeOrders(orders, { value: toBn("0.02") })
    ...
    it("should match order successfully", async () => {
      ...
      await expect(
        await esDex.connect(owner).matchOrder(order, bidOrder)
      ).to.changeEtherBalances(
        [esDex, owner, esVault],
        [toBn("0.0002"), toBn("0.0098"), toBn("-0.01")]
      );
    ...
    it("should revert if msgValue > 0", async () => {
      ...
      await expect(
        esDex
          .connect(owner)
          .matchOrder(order, bidOrder, { value: toBn("0.01") })
    ...
    it("should revert if maker is zero", async () => {
      ...
      await expect(
        esDex.connect(owner).matchOrder(order, bidOrder)
  ...
  describe("should check match successfully if msg.sender is buyOrder.maker", async () => {
    ...
    it("should match order successfully", async () => {
      ...
      await expect(
        await esDex
          .connect(addr1)
          .matchOrder(listOrder, order, { value: toBn("0.01") })
      ).to.changeEtherBalances(
        [esDex, owner, addr1],
        [toBn("0.0002"), toBn("0.0098"), toBn("-0.01")]
      );
    ...
    it("should match order with exist bid order successfully", async () => {
      ...
      await expect(
        await esDex.connect(addr1).makeOrders(orders, { value: toBn("0.04") })
      ).to.changeEtherBalances([addr1, esVault], [toBn("-0.01"), toBn("0.01")]);

      expect(await testERC721.ownerOf(0)).to.equal(await esVault.getAddress());
      await expect(
        await esDex.connect(addr1).matchOrder(listOrder, order)
      ).to.changeEtherBalances(
        [esDex, owner, esVault],
        [toBn("0.0002"), toBn("0.0098"), toBn("-0.01")]
      );
    ...
    it("should revert if maker is zero", async () => {
      ...
      await expect(
        esDex.connect(addr1).matchOrder(listOrder, order)
      ).to.be.revertedWith("HD: sender invalid");
    });
    ...
    it("should revert if buy price < sell price", async () => {
      ...
      await expect(
        await esDex.connect(addr1).makeOrders(orders, { value: toBn("0.004") })
      ).to.changeEtherBalances(
        [addr1, esVault],
        [toBn("-0.002"), toBn("0.002")]
      );
  ...
  describe("should match orders successfully", async () => {
    it("should match list orders successfully", async () => {
      ...
      await expect(await esDex.makeOrders([order2])).to.emit(esDex, "LogMake");
      ...
      successes = await esDex
        .connect(addr1)
        .matchOrders.staticCall(matchDetails, { value: toBn("0.06") });
      expect(successes[0]).to.equal(true);
      ...
      await expect(
        await esDex
          .connect(addr1)
          .matchOrders(matchDetails, { value: toBn("0.06") })
      ).to.changeEtherBalances(
        [esDex, owner, addr1],
        [toBn("0.0004"), toBn("0.0196"), toBn("-0.02")]
      );
    ...
    it("should match bid orders successfully", async () => {
      ...
      await expect(
        await esDex
          .connect(addr1)
          .makeOrders([buyOrder, buyOrder2], { value: toBn("0.04") })
      ).to.emit(esDex, "LogMake");
      ...
      successes = await esDex.matchOrders.staticCall(matchDetails);
      // console.log("successes: ", successes)
      expect(successes[0]).to.equal(true);
      expect(successes[1]).to.equal(true);

      await expect(
        await esDex.matchOrders(matchDetails)
      ).to.changeEtherBalances(
        [esDex, owner, esVault],
        [toBn("0.0008"), toBn("0.0392"), toBn("-0.04")]
      );
    ...
    describe("should transfer nft successfully", async () => {
      it("should transfer erc721 successfully", async () => {
        ...
        to = addr1.address;
        asset1 = [await testERC721.getAddress(), 0];
        asset2 = [await testERC721.getAddress(), 1];
      ...
      it("should withdraw ETH successfully", async () => {
        {
          ...
          await expect(await esDex.makeOrders([order])).to.emit(
            esDex,
            "LogMake"
          );
          ...
          await expect(
            await esDex
              .connect(addr1)
              .matchOrder(order, buyOrder, { value: toBn("3") })
          ).to.changeEtherBalances(
            [esDex, owner, addr1],
            [toBn("0.02"), toBn("0.98"), toBn("-1")]
          );
          expect(await testERC721.ownerOf(0)).to.equal(addr1.address);
        }

        await expect(
          await esDex.withdrawETH(owner.address, toBn("0.02"))
